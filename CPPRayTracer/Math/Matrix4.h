...
FINLINE Vector3 VECTORCALL ExtractPosition(const Matrix4 matrix) noexcept
{
	Vector4 row3 = matrix.r[3];
	return Vector3(row3.y, row3.z, row3.w);
}

FINLINE Vector3 VECTORCALL ExtractScale(const Matrix4 matrix) noexcept
{
...
inline Quaternion VECTORCALL ExtractRotation(const Matrix4 matrix, bool rowNormalize = true) noexcept
{
...
FINLINE Vector4 VECTORCALL Vector3Transform(const Vector3 V, const Matrix4 M) noexcept
{
